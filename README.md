...
Both the frontend and backend of this project were rigorously tested to ensure functional reliability, maintainability, and stability. The backend uses Mocha, Chai, Supertest, and NYC for comprehensive API and controller testing. It verifies all core modules, including authentication, note management, and centralized logging. Controller tests (auth.controller.spec.js, note.controller.spec.js, and log.routes.spec.js) validate success and error responses for each endpoint, including registration, login, password reset, note CRUD operations, and Pino logger routes across different log levels. Middleware and error-handling functions are tested with mock requests to confirm consistent JSON error structures and HTTP status codes. Tests are executed in an isolated environment using a mockExpress.js helper, with coverage reports stored in backend/coverage/. The frontend testing suite, implemented with Jest and React Testing Library (RTL), ensures that all components and pages render correctly and behave as expected. It covers main pages such as Login, Signup, Dashboard, and Editor, along with UI components like Navbar, NoteCard, and ProtectedRoute, validating input handling, state changes, and route protection. Integration and smoke tests confirm proper rendering and simulated API interaction, ensuring that no UI flow breaks under test conditions. All 8 frontend test suites (51 tests) passed successfully, as did all backend unit and integration tests, confirming the end-to-end stability of both layers. Coverage reports are generated for both environments (frontend/coverage/ and backend/coverage/), verifying that all major functionalities of the MERN stack application perform as intended under test conditions. 
